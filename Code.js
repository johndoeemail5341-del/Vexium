...
/** ===== AFTER-LOGIN MENU (alternate repo) =====
 * This is the UI that opens AFTER a successful password/auth.
 */
const MAIN_RAW_HTML_URL = "https://raw.githubusercontent.com/johndoeemail5341-del/Vexium/main/Password";
...
<div class="card">
  ...
  // Load the LOGIN SHELL from your main repo
...
/* ---------- Open the menu from the alternate repo ---------- */
function openMainUI(){
  try{
    const res=UrlFetchApp.fetch(MAIN_RAW_HTML_URL+"?v="+Date.now(),{muteHttpExceptions:true,followRedirects:true});
    if(res.getResponseCode()!==200){
      SpreadsheetApp.getUi().alert("Failed to load menu UI from alternate repo.");
      return false;
    }
    const html=HtmlService.createHtmlOutput(res.getContentText()).setWidth(1000).setHeight(1000).setTitle("Vexium");
    SpreadsheetApp.getUi().showModalDialog(html,"Vexium");
    ...
    SpreadsheetApp.getUi().alert("Error loading menu UI.");
    return false;
  }
}
